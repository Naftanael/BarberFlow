...
export default function ServicesPage() {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      setIsLoading(true);
      try {
        // Passa 'true' para incluir serviços inativos na listagem do painel
        const fetchedServices = await getServices(BARBERSHOP_ID, true);
  ...
  const handleOpenDialog = (service?: Service) => {
    setCurrentService(
      service || { name: '', duration: 30, price: 0, isActive: true }
    );
  ...
  const handleSaveService = async () => {
    try {
      if (currentService.id) {
        ...
        setServices(
          services.map((s) => (s.id === id ? { ...s, ...dataToUpdate } : s))
        );
        toast({
          title: 'Sucesso!',
        ...
        setServices([
          ...services,
          { ...currentService, id: newServiceId } as Service,
        ]);
        toast({
          title: 'Sucesso!',
